...
export class PerformanceMonitor {
	...
	private updateHistory(fps: number): void {
		...
		if (frameTime > this.TARGET_144FPS_BUDGET) {
			...
			if (this.performanceAlerts > 10) {
				console.warn(`Sustained frame drops detected: ${frameTime.toFixed(2)}ms > ${this.TARGET_144FPS_BUDGET}ms`);
	...
	isOptimal(targetFps = 144): boolean {
		...
		return average >= targetFps * 0.95 && consistency >= 0.90;
	}
	...
	private calculateConsistency(targetFps: number): number {
		...
		const threshold = targetFps * 0.90; // 90% of target
...
export function measureRenderPerformance(
	...
	if (duration > targetBudget) {
		...
		console.warn(`${label} budget exceeded: ${duration.toFixed(3)}ms (${budgetPercent}% of ${targetBudget.toFixed(3)}ms budget)`);
	}
