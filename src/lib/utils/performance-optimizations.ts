...
export class RAFThrottler {
  ...
  private executeCallbacks(currentTime: number): void {
    ...
    if (executionTime > PERFORMANCE_CONSTANTS.WARNING_FRAME_TIME) {
      console.warn(
        `RAFThrottler: Frame execution took ${executionTime.toFixed(2)}ms (target: ${PERFORMANCE_CONSTANTS.FRAME_BUDGET_MS}ms)`
      );
    }
  }
...
class PerformanceMonitor {
  ...
  getStats(): {
    ...
    const avgFrameTime =
      this.samples.reduce((sum, s) => sum + s.frameTime, 0) / this.samples.length;
...
export const currentFPS = derived(performanceMonitor.metrics, ($metrics) => $metrics?.fps || 0);
...
export function measurePerformance<T>(
  ...
  if (duration > PERFORMANCE_CONSTANTS.WARNING_FRAME_TIME) {
    console.warn(
      `${label} took ${duration.toFixed(2)}ms (target: <${PERFORMANCE_CONSTANTS.FRAME_BUDGET_MS}ms)`
    );
  }
