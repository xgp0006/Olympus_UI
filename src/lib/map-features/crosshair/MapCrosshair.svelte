...
<script lang="ts">
  ...
  function renderCrosshair(timestamp: number): void {
    ...
    if (gridReference) {
      ...
      crosshairCtx.fillRect(centerX - boxWidth / 2, centerY + size / 2 + 10, boxWidth, boxHeight);
  ...
  function renderRing(timestamp: number): void {
    ...
    // Render ring
    ringRenderer.render(viewport, currentRingDistance, settings.ringUnits || 'meters', {
      color: '#00ff88',
      strokeWidth: 2,
      opacity: 0.8,
      labelColor: '#ffffff',
