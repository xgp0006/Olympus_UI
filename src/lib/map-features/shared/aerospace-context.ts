...
import { renderSuccess, renderError } from './aerospace-pools';
import type { RenderResult, RenderError } from './aerospace-pools';
...
export class AerospaceCanvasContext {
  ...
  safeClearRect(x: number, y: number, width: number, height: number): RenderResult<void> {
    try {
      ...
      if (
        ...
        return renderError(RenderErrorType.INVALID_PARAMETERS, 'Invalid clearRect parameters', {
          x,
          y,
          width,
          height
      ...
      return renderError(RenderErrorType.CANVAS_ERROR, `clearRect failed: ${error}`, {
        x,
        y,
        width,
        height,
  ...
  safeGetImageData(x: number, y: number, width: number, height: number): RenderResult<ImageData> {
    try {
      ...
      if (clampedWidth <= 0 || clampedHeight <= 0) {
        return renderError(RenderErrorType.INVALID_PARAMETERS, 'Invalid image data dimensions', {
          x,
          y,
          width,
          height,
      ...
      if (!imageData || !imageData.data || imageData.data.length === 0) {
        return renderError(RenderErrorType.CANVAS_ERROR, 'getImageData returned invalid data');
      }

      return renderSuccess(imageData);
    } catch (error) {
      return renderError(RenderErrorType.CANVAS_ERROR, `getImageData failed: ${error}`, {
        x,
        y,
        width,
        height,
  ...
  safeMeasureText(text: string, font?: string): RenderResult<TextMetrics> {
    try {
      ...
      if (text.length > 1000) {
        // Reasonable limit
        return renderError(RenderErrorType.INVALID_PARAMETERS, 'Text too long for measurement', {
      ...
      if (font) {
        ...
        try {
          ...
          return renderError(RenderErrorType.INVALID_PARAMETERS, `Invalid font: ${font}`, {
            font,
            error: fontError
          });
        }
      ...
      return renderError(RenderErrorType.CANVAS_ERROR, `measureText failed: ${error}`, {
        text,
        font,
        error
      });
  ...
  safeStroke(path?: Path2D): RenderResult<void> {
    try {
      ...
      return renderError(RenderErrorType.CANVAS_ERROR, `stroke failed: ${error}`, {
        hasPath: !!path,
        error
      });
    }
  ...
  safeFillRect(x: number, y: number, width: number, height: number): RenderResult<void> {
    try {
      ...
      if (
        ...
        return renderError(RenderErrorType.INVALID_PARAMETERS, 'Invalid fillRect parameters', {
          x,
          y,
          width,
          height
      ...
      return renderError(RenderErrorType.CANVAS_ERROR, `fillRect failed: ${error}`, {
        x,
        y,
        width,
        height,
  ...
  safeFillText(text: string, x: number, y: number, maxWidth?: number): RenderResult<void> {
    try {
      ...
      if (!Number.isFinite(x) || !Number.isFinite(y)) {
        return renderError(RenderErrorType.INVALID_PARAMETERS, 'Invalid position parameters', {
          x,
          y
        });
      }
      ...
      if (maxWidth !== undefined && (!Number.isFinite(maxWidth) || maxWidth <= 0)) {
        return renderError(RenderErrorType.INVALID_PARAMETERS, 'Invalid maxWidth parameter', {
          maxWidth
        });
      }
      ...
      return renderError(RenderErrorType.CANVAS_ERROR, `fillText failed: ${error}`, {
        text,
        x,
        y,
        maxWidth,
  ...
  attemptContextRecovery(): RenderResult<void> {
    try {
      ...
      return renderError(RenderErrorType.CONTEXT_LOST, `Context recovery failed: ${error}`, {
        error
      });
    }
  }
 ...
 */
export function createAerospaceContext(
  canvas: HTMLCanvasElement
): RenderResult<AerospaceCanvasContext> {
  try {
    ...
    return renderError(RenderErrorType.CONTEXT_LOST, `Context creation failed: ${error}`, {
      error
    });
  }
}
