...
<script lang="ts">
  ...
  function handleClick(event: MouseEvent) {
    ...
    const lat = viewport.center.lat + (y - canvasHeight / 2) * (viewport.bounds.north - viewport.bounds.south) / canvasHeight;
    const lng = viewport.center.lng + (x - canvasWidth / 2) * (viewport.bounds.east - viewport.bounds.west) / canvasWidth;
  ...
  function drawMeasurementLabel() {
    ...
    if (label) {
      ...
      ctx.fillText(
        label,
        lastPoint.x,
        lastPoint.y - POINT_RADIUS - boxHeight / 2 - 5
      );
    }
  }
  ...
  export function clear() {
    ...
    currentArea = 0;
    isDrawing = false;
    if (ctx && canvas) { // NASA JPL Rule 6: Guard both context and canvas
      ctx.clearRect(0, 0, canvas.width || 0, canvas.height || 0);
    }
  }
