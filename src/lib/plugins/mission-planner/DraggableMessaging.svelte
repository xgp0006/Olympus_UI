...
<script lang="ts">
  ...
  import {
    ...
    removeNotification,
    clearNotifications
  } from '$lib/map-features/messaging/notification-store';
  ...
  $: minimizedCoinProps = {
    item: {
      id: 'messaging',
      type: 'loiter',
      name: `${$notifications.length} Msgs`,
      params: {},
  ...
  function handleCoinExpand(event: CustomEvent<{ itemId: string }>) {
    ...
    console.log('Expanding messaging from coin');
  }
  ...
  $: filteredNotifications = $notifications.filter(
    (n: import('$lib/map-features/messaging/notification-store').Notification) =>
