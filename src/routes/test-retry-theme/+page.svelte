...
<div style="padding: 2rem; background: #111; color: #fff; min-height: 100vh; font-family: monospace;">
  <h1>Theme Loading with Retry Test</h1>
  
  <h2>Status: {status}</h2>
  
  <h3>Logs:</h3>
  <div style="background: #222; padding: 1rem; border-radius: 4px; max-height: 400px; overflow-y: auto;">
    {#each logs as log}
  ...
  {#if theme}
    ...
    <details>
      ...
      <pre style="background: #222; padding: 1rem; overflow: auto; max-height: 400px;">{JSON.stringify(theme, null, 2)}</pre>
    </details>
  {/if}
