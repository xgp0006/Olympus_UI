...
export class MissionControl extends EventEmitter {
  ...
  public async start(): Promise<void> {
    ...
    try {
      ...
      throw new Error(`Failed to start Mission Control: ${errorMessage}`);
    }
  }
  ...
  public async mergeAllWorktrees(targetBranch: string): Promise<MergeResult> {
    ...
    const worktrees = this.agentManager
      .getActiveAgents()
      .map((agent: ClaudeAgent) => agent.worktree);
  ...
  public getStatus(): {
    running: boolean;
    agents: Array<{ id: string; status: string; taskId?: string }>;
    activeTasks: number;
