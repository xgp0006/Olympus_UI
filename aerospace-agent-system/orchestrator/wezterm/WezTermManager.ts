...
export class WezTermManager extends EventEmitter {
   ...
   */
  public async getPaneOutput(paneId: string, lines: number = 50): Promise<string[]> {
    try {
      const result = await this.executeWezTermCommand([
        'cli',
        'get-text',
   ...
   */
  public getActivePanes(): WezTermPane[] {
  ...
  public async saveLayout(name: string): Promise<void> {
    const layoutConfig = {
      ...
      panes: Array.from(this.panes.entries()).map(([id, pane]) => ({
        id,
        ...pane
      }))
    };
