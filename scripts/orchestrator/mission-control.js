...
async function startMissionControl() {
  ...
  // Save PID
  await fs.writeFile(
    path.join(ORCHESTRATOR_DIR, 'mission-control.pid'),
    child.pid.toString()
  );
...
program
  .command('stop')
  .description('Stop Mission Control orchestrator')
  .action(stopMissionControl);

program
  .command('status')
  .description('Show Mission Control status')
  .action(showStatus);
...
program
  .command('validate')
  .description('Validate all agent worktrees')
  .action(validateAll);
